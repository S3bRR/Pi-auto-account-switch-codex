...
The published repository contains the runtime source only. Before release, version 1.1.1 was typechecked and exercised in real Pi 0.87.1 processes with synthetic credentials covering successful continuation, consecutive exhausted accounts, all-account exhaustion, short throttling, manual mode, startup behavior, SSE enforcement, stale-token replacement, request-local quota isolation, and credential import persistence.
