...
Built for **@earendil-works/pi-coding-agent 0.87.1** (Node 22+). Other Pi versions and OpenAI error variants have not been verified; see [Limitations](#limitations).
...
**Detection is intentionally bounded.** The extension uses a structured `usage_limit_reached` code when Pi exposes one. Pi 0.87.1 can hide that code on WebSocket failures or normalize both quota and rate errors to the same friendly sentence, so the extension also recognizes the exact terminal wording `The usage limit has been reached` and plan-scoped reset messages of at least ten minutes. It does not switch on a generic 429, short transient throttling, network failures, server errors, context overflow, cancellation, billing errors, or tool failures. When supplied, the actual `resets_at` is displayed; otherwise reset is “unknown.” Blocks remain for the process lifetime (including after reset); no background polling or automatic unblocking is attempted. A candidate whose credentials cannot refresh is skipped and marked for reauthentication without launching OAuth. Unsupported models/entitlement errors are not treated as quota exhaustion.
...
The extension was validated locally in real Pi 0.87.1 sessions with synthetic Codex quota responses, including Pi's normalized plan-limit message, before publishing this product repository; automated test sources are not included. To check the TypeScript source in a development checkout, run `npm ci && npm run typecheck`. A real human-operated TUI, real OpenAI quota responses/WebSockets, real token rotation, multi-process refresh races, subagent continuation, and compaction recovery have **not** been verified against live services. Automatic failover cannot be guaranteed for every error or subagent session.
