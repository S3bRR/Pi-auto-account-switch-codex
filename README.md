...
- The extension uses Codex's SSE transport so structured quota responses can be inspected reliably. Tokens and response bodies are never logged or persisted by the extension.
...
The published repository contains the runtime source only. Before release, version 1.1.0 was typechecked and exercised in real Pi 0.87.1 processes with synthetic credentials covering successful continuation, consecutive exhausted accounts, all-account exhaustion, short throttling, manual mode, startup behavior, SSE enforcement, and credential import persistence.
